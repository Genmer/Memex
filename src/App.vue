...
<template>
  ...
  <div v-else class="flex h-screen w-full text-white/90 selection:bg-indigo-500/30 overflow-hidden">
    ...
    <main class="app-main flex-1 flex flex-col min-w-0 bg-white/5 backdrop-blur-md relative z-10 shadow-[-10px_0_30px_rgba(0,0,0,0.5)] border-l border-white/10 relative transition-colors duration-200">
      ...
      <div class="flex-1 overflow-y-auto p-8 relative">
        ...
        <div v-if="activeView.includes('skills') || activeView.includes('memories')" class="flex items-center gap-3 mb-6 flex-wrap">
          <button 
            ...
            :class="favoriteOnly ? 'bg-amber-500/15 text-yellow-300 border-amber-500/30' : 'bg-white/5 text-white/50 border-white/10 hover:bg-white/10 hover:text-white/80'"
          >
          ...
          <div class="flex items-center gap-1 bg-white/5 rounded-lg p-1 border border-white/10">
            <button 
              ...
              :class="sortBy === opt.value ? 'bg-indigo-500/20 text-indigo-200 font-medium' : 'text-white/50 hover:text-white/80'"
            >
              {{ opt.label }}
            </button>
          </div>
          ...
          <button
            ...
            class="px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors flex items-center gap-1.5"
            :class="isBatchMode ? 'bg-indigo-600 text-white border-indigo-500 shadow-sm' : 'bg-white/5 text-white/60 border-white/10 hover:bg-white/10 hover:text-white'"
          >
            <CheckSquare :size="13" />
          ...
          <div v-if="activeView.includes('memories')" class="flex items-center gap-1 bg-white/5 rounded-lg p-1 border border-white/10">
            <button
              ...
              :class="memoryClusterMode === 'flat' ? 'bg-indigo-500/20 text-indigo-200 font-medium' : 'text-white/50 hover:text-white/80'"
            >
              <LayoutGrid :size="12" />
              <span>平铺</span>
            </button>
            <button
              ...
              class="px-2.5 py-1 rounded-md text-xs transition-colors flex items-center gap-1"
              :class="memoryClusterMode === 'project' ? 'bg-indigo-500/20 text-indigo-200 font-medium' : 'text-white/50 hover:text-white/80'"
            >
              <FolderTree :size="12" />
              <span>按项目聚类</span>
            </button>
        ...
        <div v-else-if="activeView.includes('skills')" class="h-full">
          ...
          <div v-if="filteredSkills.length > displaySkillLimit" class="pt-2 pb-12 flex justify-center">
            <button 
              ...
              class="px-5 py-2.5 rounded-xl bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-300 border border-indigo-500/30 text-xs font-medium transition-colors shadow-lg"
            >
          ...
          <div v-if="!filteredSkills.length" class="h-full flex flex-col items-center justify-center py-20 opacity-80">
            ...
            <h3 class="text-xl font-medium tracking-wide text-white">{{ t('hub.empty.title') }}</h3>
          ...
          <div class="bg-black/20 backdrop-blur-xl p-8 rounded-2xl border border-white/10 shadow-2xl">
            ...
            <div class="space-y-4">
              <div class="flex items-center justify-between mb-2">
                ...
                <button @click="addTarget" class="px-3 py-1.5 bg-indigo-500/20 text-indigo-300 hover:bg-indigo-500/30 rounded-lg text-xs font-medium border border-indigo-500/30 transition-colors flex items-center gap-1.5">
                  <Folder :size="14" />
                  添加路径
                </button>
              </div>
      ...
      <div v-if="showWelcomePrompt" class="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
        <div class="bg-[#0f1117]/95 border border-white/15 p-8 rounded-2xl shadow-2xl max-w-md w-full animate-in fade-in zoom-in-95 duration-300">
          ...
          <h3 class="text-2xl font-semibold mb-3 text-white">{{ t('prompt.title') }}</h3>
    ...
    >
      ...
      >
        ...
        <!-- Expanded Floating HUD Card (Visible when user navigates away or keeps expanded) -->
        <div 
          v-else-if="!activeView.includes('skills')"
          class="w-80 p-4 rounded-2xl bg-[#161922]/95 backdrop-blur-2xl border border-indigo-500/40 text-white shadow-[0_10px_40px_rgba(0,0,0,0.8)] space-y-3 animate-in fade-in zoom-in-95 duration-200"
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2 min-w-0">
              ...
              <span class="text-xs font-semibold text-white/90 truncate">AI 批量解析任务</span>
            </div>
